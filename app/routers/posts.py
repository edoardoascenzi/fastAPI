...
@router.get("", response_model=List[schemas.ResponsePostLike])
...
@router.get("/latest", response_model=schemas.ResponsePostLike)
...
@router.get("/{id}" , response_model=schemas.ResponsePostLike) #path parameter
...
def update_Post(id:int, post: schemas.UpdatePost , db: Session = Depends(get_db), currentUser = Depends(oauth2.getCurrentUser)): #with the PUT we need all the field
    ...
    if updatePost == None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Post with id {id} does not exist")
    print(updatePost.user_id, currentUser.id)
